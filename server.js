...
app.get('/privacy', (req, res) => {
  const privacyText = `
    ...
    Сервис «Докуда» (ИП Озерская С.А.)

    1. Мы собираем только те данные, которые необходимы для оформления и доставки заказа: имя, телефон, email, адрес.
    2. Данные используются исключительно для связи с вами и выполнения услуг.
    3. Мы не передаём ваши личные данные третьим лицам, за исключением случаев, предусмотренных законодательством РФ.
    4. Платёжные данные (карты) обрабатываются банком-партнёром напрямую и не сохраняются на наших серверах.
    5. Вы можете в любой момент запросить удаление ваших данных, написав на dokuda.dostavka@gmail.com.
    6. Приложение может использовать геолокацию для расчёта стоимости доставки (с вашего разрешения).
    7. Мы используем push-уведомления для оповещения о статусе заказа. Вы можете отключить их в настройках устройства.
    8. Актуальная версия политики всегда доступна по этому адресу.

    Контакты: dokuda.dostavka@gmail.com, +7-921-434-27-66
  `;
  res.send(`<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><title>Политика конфиденциальности — Докуда</title></head><body><pre style="white-space: pre-wrap; font-family: Arial; padding: 20px;">${privacyText}</pre></body></html>`);
});
